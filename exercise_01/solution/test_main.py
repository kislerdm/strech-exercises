...
def test_data_quality():
    required_files = {"users.csv", "transactions.csv", "result.csv"}

    base_dir = os.getenv("BASE_DIR", "/data")

    if required_files.intersection(set(os.listdir(base_dir))) != required_files:
        pytest.skip("no required fixtures (%s) found in %s" % (",".join(required_files), base_dir))

    with open(f"{base_dir}/result.csv", "r") as file:
        want = file.read()
